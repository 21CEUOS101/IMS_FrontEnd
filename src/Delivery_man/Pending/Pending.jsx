...
const Pending = () => {
  ...
  const [currentOrderType, setCurrentOrderType] = useState('order'); // Default order type
  ...
  if (orderData === null || w2wOrderData === null || returnOrderData === null) {
    return <div>Loading...</div>; // or return a loading indicator
  }

  const combinedData = currentOrderType === 'order' ? orderData : currentOrderType === 'w2worder' ? w2wOrderData : returnOrderData;
  ...
  currentData.length === 0 ? (
    <h1>No items available for {currentOrderType === 'order' ? 'Customer Order' : currentOrderType === 'w2worder' ? 'W2W Order ' : 'Return Order '}</h1>
  ) : (
