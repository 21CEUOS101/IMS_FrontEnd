...
function HoverCardWithHeaderExample({ data }) {
  const { product, user, supplier } = data;
  ...
  return (
    ...
    >
      <Card.Header style={{ position: 'relative', backgroundColor: randomBackgroundColor, color: '#000', fontSize: "1.2rem", display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 1rem', height: "40px" }}>
        ...
        <span style={{ marginLeft: "500px" }}>Product no. :- {product?.id}</span>
      </Card.Header>
      <Card.Body style={{ display: 'flex', flexDirection: 'row', background: "#FFFAFA" }}>
        <div style={{ flex: 1 }}>
          ...
          <Card.Text style={{ color: '#000', fontSize: "1rem" }}><span style={{ fontStyle: "italic" }}>Supplier Address :  {supplier.address}, {supplier.pincode}</span></Card.Text>
        </div>
        <div style={{ flex: 0.5 }}>
          <Card.Text style={{ color: '#000', fontSize: "1rem" }}><span style={{ fontStyle: "italic" }}>Product name : {product.name}</span></Card.Text>
          <Card.Text style={{ color: '#000', fontSize: "1rem" }}><span style={{ fontStyle: "italic" }}>Product Wholesale Price :{product.whole_sale_price}</span></Card.Text>
        </div>
        <div style={{ flex: 0.3 }}>
          <Button
            style={{
      ...
      <RequestDetails isOpen={isDialogOpen} handleClose={handleCloseDialog} details={selectedOrder} />
    </Card>
  );
}
