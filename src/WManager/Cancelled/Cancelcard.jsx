...
import CancelCardDetails from './CancelCardDetails'  
import {makeSupplierOrderByWId} from '../../Services/Manager'
import {getallcancelledByMid} from '../../Services/Supplier'
  ...
  return (
    ...
    >
      ...
      <Card.Body style={{ display: 'flex', flexDirection: 'row', background: "#FFFAFA" }}>
        ...
        <div style={{ flex: 0.3 }}>
        Status: <span style={{fontStyle:"italic",color:"blue"}}> {supplyorder?.status === "cancel" ? "Cancelled" :"pending"}</span>
          <Button
            style={{
      ...
      <CancelCardDetails isOpen={isDialogOpen} handleClose={handleCloseDialog} details={selectedOrder} />
    </Card>
  );
}
